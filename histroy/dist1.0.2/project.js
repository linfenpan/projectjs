/*! By da宗熊 2015-10-26 v1.0.2 https://github.com/linfenpan/projectM.git */
;(function(window){
...
function config(pro){
    ...
    if(base){
        if(!path.isAbsolute(base)){
            data.basePath = path.join(path.dir(window.location.href), base);
        }
    }else{
...
function requireOne(url, callback){
    ...
    if(def && def.loaded == 1){
        ...
        fn(url, function(res, isFromScript){
            // res != null 是 loaded = -1 这种情况的，其它脚本 loaded = 0 & res = null
            if(isFromScript){
                res = res || defineRes;
                defineRes = null;
                if(isFunction(res)){
